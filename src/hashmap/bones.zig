...
pub fn HashFn(comptime StateType:type, comptime Key:type) type {
    return *const fn(StateType, Key) u64;
}

pub fn HashMap(comptime Key:type, comptime Val:type, comptime StateType:type, hash_fn:HashFn(StateType, Key)) type {
    return struct {
        bucket:[]Entry,
        size:u64,
        hash_num:u64 = 5381,
        ...
        pub const hashFn = hash_fn;

        const Self = @This();

        pub fn init(state:StateType) !Self {
            return .{
                .bucket = alloc.alloc(Entry, 0) catch unreachable,
                .size = 0,
                .state = state,
            };
        }
